...
Public Class E_Chkt
    ...
    Dim ro As Integer
    Dim dat2 As Date
    Dim erly_chrg As Integer = 0
    Dim dis_rate As Integer = 0
    Dim ds As New DataSet
    Private Sub E_Chkt_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Dim obj As New Data
        dt = obj.data_fill("select * from Room_Allocation Where Check_Out > '" & Date.Now.ToString("yyyy-MM-dd") & "'  and Check_In <=  '" & Date.Now.ToString("yyyy-MM-dd") & "' and Invoice_No in (select Invoice_No from Check_In) ")
        If dt.Rows.Count = 0 Then
            MessageBox.Show("No Guests")
        Else
    ...
    Private Sub cmb_rmno_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmb_rmno.SelectedIndexChanged
        ...
        If cmb_rmno.SelectedIndex <> -1 Then
            ...
            If f = True Then
                Dim obj As New Data
                dt = obj.data_fill("select * from Room_Allocation Where Room_No = '" & cmb_rmno.Text & "' and  Check_Out > '" & Date.Now.ToString("yyyy-MM-dd") & "'  and Check_In <=  '" & Date.Now.ToString("yyyy-MM-dd") & "' and Invoice_No in (select Invoice_No from Check_In) ")
                ...
                ro = dt.Rows(0)("Row_Id")
                txt_chkout.Text = Today.ToString("dd-MM-yyyy")
                ...
                erly_chrg = 600 * Val(dt.Rows(0)("Person_Stay"))
                Dim tot As Integer = Val(dt.Rows(0)("Total_Amount")) + Val(txt_srvc.Text) + erly_chrg
                dgv_bill.Rows.Add(cmb_rmno.Text, stay, dis_rate, "@10%", "@5%", txt_srvc.Text, erly_chrg, tot, dt.Rows(0)("Amount_Paid"), tot - Val(dt.Rows(0)("Amount_Paid")))
            End If
        ...
        If cmb_pay_mode.SelectedIndex <> -1 And cmb_rmno.SelectedIndex <> -1 Then
            ...
            Try
                ...
                If IsDBNull(dt.Rows(0)("Payment_Mode")) Then
                    obj.insert_data("insert into H_Sales values(" & id & ",'" & txt_name.Text & "','" & dt.Rows(0)("Invoice_No") & "','" & Today.ToString("yyyy-MM-dd") & "','" & dgv_bill.Rows.Item(0).Cells("Room_No").Value & "','" & dt.Rows(0)("Room_Type") & "', '" & dt.Rows(0)("Person_Stay") & "' ,'" & dat1.ToString("yyyy-MM-dd") & "','" & dat2.ToString("yyyy-MM-dd") & "',  '" & dt.Rows(0)("Total_Amount") & "', '" & dgv_bill.Rows.Item(0).Cells("Discount").Value & "','" & dgv_bill.Rows.Item(0).Cells("Room_Service").Value & "','" & dgv_bill.Rows.Item(0).Cells("Total").Value & "', '@10%' , '@5%','" & dgv_bill.Rows.Item(0).Cells("Paid").Value & "','" & dgv_bill.Rows.Item(0).Cells("Pay").Value & "',  '" & cmb_pay_mode.Text & "','" & erly_chrg & "' )  ")
                Else
                    ...
                    obj.insert_data("insert into H_Sales values(" & id & ",'" & txt_name.Text & "','" & dt.Rows(0)("Invoice_No") & "','" & dat2.ToString("yyyy-MM-dd") & "','" & dgv_bill.Rows.Item(0).Cells("Room_No").Value & "','" & dt.Rows(0)("Room_Type") & "', '" & dt.Rows(0)("Person_Stay") & "' ,'" & dat1.ToString("yyyy-MM-dd") & "','" & dat2.ToString("yyyy-MM-dd") & "',  '" & dt.Rows(0)("Total_Amount") & "', '" & dgv_bill.Rows.Item(0).Cells("Discount").Value & "','" & dgv_bill.Rows.Item(0).Cells("Room_Service").Value & "','" & dgv_bill.Rows.Item(0).Cells("Total").Value & "', '@10%' , '@5%','" & dgv_bill.Rows.Item(0).Cells("Paid").Value & "','" & dgv_bill.Rows.Item(0).Cells("Pay").Value & "', '" & pay_mode & "','" & erly_chrg & "')  ")
                End If
        'Next
        Catch ex As Exception
        Dim obj As New Data
                MessageBox.Show(ex.Message)
                MessageBox.Show("Exception in insert query")
                chk1 = False
            ...
            If chk1 = True Then
                Try
                    Dim obj As New Data
                    obj.insert_data("delete from Room_Allocation where Row_Id = '" & ro & "'  ")
                Catch ex As Exception
                    MessageBox.Show(ex.Message)
                    chk2 = False
                End Try
    ...
    Private Sub cmb_rmno_KeyPress(sender As Object, e As KeyPressEventArgs) Handles cmb_rmno.KeyPress, txt_srvc.KeyPress, cmb_pay_mode.KeyPress
